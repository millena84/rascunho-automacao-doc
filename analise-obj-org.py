...
def buscar_bool(root, tag):
    return buscar_tag(root, tag).lower() == 'true'
        ...
        for arq in os.listdir(pasta_completa):
            ...
            try:
                ...
                for perm in root.findall('.//ns:objectPermissions', ns):
                    obj = buscar_tag(perm, 'object')
                    if not obj:
                        continue
                    label_obj = mapa_labels_objetos.get(obj, obj)
                    linha = [
                        label_obj, obj, nome_perm, nome_perm,
                        buscar_tag(perm, 'allowCreate'), buscar_tag(perm, 'allowDelete'),
                        buscar_tag(perm, 'allowEdit'), buscar_tag(perm, 'allowRead'),
                        buscar_tag(perm, 'modifyAllRecords'), buscar_tag(perm, 'viewAllFields'), buscar_tag(perm, 'viewAllRecords')
                    ]
                    saidas[chave_csv_obj].append(linha)

                for perm in root.findall('.//ns:fieldPermissions', ns):
                    campo = buscar_tag(perm, 'field')
                    if '.' not in campo:
                        continue
                    ...
                    linha = [
                        ...
                        buscar_tag(perm, 'editable'), buscar_tag(perm, 'readable'), 'true'
                    ]
                    saidas[chave_csv_field].append(linha)
            except Exception as e:
                print(f"⚠️ Erro ao processar {tipo} {arq}: {e}")

# === AQUI virá a função processar_objetos COMPLETA da versão estável com a chamada à processar_permissoes no final ===

# (A função processar_objetos completa será inserida agora, com a chamada a processar_permissoes integrada corretamente)
...
if __name__ == "__main__":
    ...
    PASTA_PROJSF = (
        CONFIG.get('consultasTabelasInternas', {})
        .get('consultaEntityDefinition', {})
        .get('dirPosixProjSf')
    )
