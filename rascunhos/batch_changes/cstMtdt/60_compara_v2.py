...
def camel_case(s):
    return re.sub(r'[^a-zA-Z0-9]', '', ''.join(w.capitalize() for w in s.strip().split('_')))
...
try:
    with open(config_path, 'r', encoding='utf-8') as json_file:
        ...
        config_custom = config.get('infoEspecificaProcessos', {}).get('avaliacaoCustomMdt', [])[0].get('infoCopiaLocal', {})
except Exception as e:
    print(f"⚠️ Erro ao ler {config_path}: {e}")

prefixo_nome = config_custom.get('filtroNomeArquivosAlvo', 'CamposCustom')
campo_filho = config_custom.get('campoRelFilho', '')
campo_pai = config_custom.get('campoRelPai', '')
campos_tela = config_custom.get('camposTela', '')
tela_utilizada = config_custom.get('tela', '')
objeto = config_custom.get('objeto', '')
...
for linha in tabela:
    ...
    if canal and formato:
        referencia.append({'canal': canal, 'formato': formato, 'tem_dado_espec': tem_dado_espec})
...
with open(saida_alteracao, 'w', encoding='utf-8-sig', newline='') as f_out, \
     ...
     open(saida_sem_alteracao, 'w', encoding='utf-8-sig', newline='') as f_inalt:
    ...
    writer_inalt = csv.writer(f_inalt, delimiter=';')
    ...
    for row in origem:
        ...
        candidatos = [ref for ref in referencia if ref['canal'].strip() == canal_xml and (ref['canal'], ref['formato']) not in usados]
        ...
        similares = sorted(candidatos, key=lambda r: -similaridade(r['formato'], formato_xml))
    ...
    # Geração de CustomMetadata a criar
    contador = 0
    for item in referencia:
        if item['tem_dado_espec'] and (item['canal'], item['formato']) not in usados:
            nome_base = f"{prefixo_nome}__{camel_case(item['formato'])}"
            nome_custom = f"{nome_base[:36]}__{contador:03d}-md-meta.xml"
            nome_label = f"{item['canal']}__{item['formato']}"
            writer_cria.writerow([
                nome_custom, nome_label, campo_filho, campo_pai, campos_tela,
                item['canal'], item['formato'], objeto, tela_utilizada
            ])
            contador += 1
