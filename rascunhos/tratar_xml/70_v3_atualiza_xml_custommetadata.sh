...
CSV_CAMINHO="./3_saida_csv/1_listaCustomAlteracao.csv" XML_ENTRADA_DIR="./2_entrada_xml" XML_SAIDA_DIR="./4_xml_corrigido_shell"
...
do ORIGEM="$XML_ENTRADA_DIR/$ARQUIVO" DESTINO="$XML_SAIDA_DIR/$ARQUIVO"

if [[ -f "$ORIGEM" ]]; then cp "$ORIGEM" "$DESTINO"
...
else echo "❌ Arquivo não encontrado: $ARQUIVO" fi
